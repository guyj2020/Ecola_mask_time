...
class E2EDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.num_samples_per_file != self.num_samples:
            ...
            # try:
            #     self.data = self.read_training_json_file(os.path.join(self.pretrain_input_file_dir, f'training_data.json')) # for duee
            # except:
            #     self.data = self.read_training_json_file(os.path.join(self.pretrain_input_file_dir, f'train_instances.json')) # for filtered yago
            sample = self.data[item]
    ...
    def apply_mask_strategy(self, token_ids, time_idx, mask_opt):
        if mask_opt == 0:
            ...
            words, words_mlm_labels = create_mlm_instances(token_ids[0: -3], self.word_mask_index,
                                                           self.word_vocab_size, self.tokenizer, mask_opt)
            ...
            words = token_ids[0: -3]
            words_mlm_labels = np.linspace(-100, -100, len(words)).astype(int).tolist()
            ...
            mask_i = random.randint(0,2)
            if mask_i == 0:
                words, words_mlm_labels = create_mlm_instances(token_ids[0: -3], self.word_mask_index,
                                                                self.word_vocab_size, mask_opt, self.tokenizer)
            elif mask_i == 1:
                entities, entities_mlm_labels = create_mlm_instances(token_ids[-3: -1], self.entity_vocab_size - 1,
                                                                self.entity_vocab_size,  mask_opt)
            elif mask_i == 2:
                relation, relation_mlm_labels = create_mlm_instances([token_ids[-1]], self.relation_vocab_size - 1,
                                                                 self.relation_vocab_size, mask_opt)
        elif mask_opt == 2 or 3:
            words, words_mlm_labels = create_mlm_instances(token_ids[0: -4], self.word_mask_index,
                                                                self.word_vocab_size, mask_opt, self.tokenizer)
    ...
    def collate_fn(self, batch):
        ...
        for instance in batch:
            ...
            token_type_ids = [0] * (num_tokens - 4) + [1] * 2 + [2] + [3]
            data.append({
                'input_ids': input_ids,
        ...
        for i in range(len(batch_num_tokens)):
            ...
            num_word_tokens = batch_num_tokens[i] - 4
