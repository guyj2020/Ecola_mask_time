...
class E2EBertTKG(BertForMaskedLM):
    ...
    def __init__(self, config, ent_num, rel_num, time_num=2592, se_prop=0.68, drop_out=0.4, ent_emb=None, rel_emb=None, tkg_model=None, tkg_type='DE', dataset='GDELT', loss_lambda=0.1, ablation=0):
        ...
        if ent_emb is not None:
            ...
            self.ent_embeddings = nn.Embedding(ent_num, config.hidden_size)
            self.ent_embeddings.weight.requires_grad = True

        if rel_emb is not None:
            self.rel_embeddings = nn.Embedding.from_pretrained(rel_emb, freeze=False)
        else:
            self.rel_embeddings = nn.Embedding(rel_num, config.hidden_size)
        ...
        self.time_w = nn.Linear(self.temporal_emb_dim, config.hidden_size, bias=False)
        if self.ablation == 1:
    ...
    def load_pretrained_tkg_embs(self, tkg_model):
        ...
        # temporal embeddings for entities
        if self.dataset == 'GDELT' or self.dataset == 'DuEE':
        ...
        elif self.dataset == 'Wiki' or self.dataset == 'TSQA':
    ...
    def create_time_embeds(self):
        # frequency embeddings
        if self.dataset == 'Wiki' or self.dataset == 'TSQA':
    ...
    def forward(self, input_ids=None, num_tokens=None, attention_mask=None, token_type_ids=None, inputs_embeds=None,
                word_masked_lm_labels=None, entity_masked_lm_labels=None, relation_masked_lm_labels=None, time_masked_lm_labels=None, tkg_tuple=None,
                tuple_labels=None):
        ...
        if self.ablation == 0 or self.ablation == 2 or self.ablation >= 3:
            ...
            entity_embeddings = self.get_tkgs_added_ent_embs(input_ids[:, num_word_tokens: num_word_tokens+2],\
                h_embs1.view(batch_size,-1,self.bert_hidden_size)[:,0], t_embs1.view(batch_size,-1,self.bert_hidden_size)[:,0])
            relation_embeddings = self.get_tkgs_added_rel_embs(input_ids[:, -2],\
                r_embs1.view(batch_size,-1,self.bert_hidden_size)[:,0], r_embs2.view(batch_size,-1,self.bert_hidden_size)[:,0]).unsqueeze(1)            
        ...
        # MLM loss
        # concat the 4 parts of embeddings
        time_embeddings = self.get_utee_time_embedd(input_ids[:, -1]).unsqueeze(1)
        time_embeddings = self.time_w(time_embeddings)
        ...
        time_prediciton_socre = self.time_lm_head(sequence_output)
        time_prediction = torch.argmax(time_prediciton_socre[:, -1 , :], dim=-1).unsqueeze(1)
        # time_prediction = torch.mul(time_prediction, 15)
        time_lm_loss = loss_fct(time_prediciton_socre.view(-1, self.time_num),
        ...
        mlm_loss = mlm_loss + time_lm_loss
