...
import sys
sys.path.insert(0, '../TKB_finetuning_3')
sys.path.insert(0, '../TKB_finetuning_2')
...
def parse_args():
    ...
    parser.add_argument('--kg_model_chkpnt', type=str,
                        default=None,
                        # default="../model/ckpts_finetuning_2/768_baseline/DE_SimplE_bs256_ne20_lr0.0005_nr200_seprop0.68_1.chkpnt",
...
def parse_batch_x(batch_x:dict, device):
    ...
    time_masked_lm_labels = batch_x['time_masked_lm_labels'].to(device)
    ...
    return input_ids, num_tokens, attention_mask, token_type_ids, word_masked_lm_labels, entity_masked_lm_labels, \
           relation_masked_lm_labels, time_masked_lm_labels, tkg_tuple, tuple_labels
...
def parse_batch_y(batch_y: dict, device):
    ...
    time_masked_lm_labels = batch_y['time_masked_lm_labels'].to(device)
    return word_masked_lm_labels, entity_masked_lm_labels, relation_masked_lm_labels, time_masked_lm_labels
...
def replace_batch_data(facts, head_or_tail, num_ent, device, tkg_type, dataset):
    ...
    if tkg_type == 'DE':
        ...
        if len(str(int(replaced_data[:, 3][0]))) == 8: #YYYYMMDD
...
def main():
    ...
    if not debug_mode:
        ...
        args.log_file_dir =  '../logs/debug/'
    mlflow.end_run()
    start_run = mlflow.start_run(run_name=args.exp)
    mlflow_logger.args_summary(args)
    ...
    if args.do_eval:
        ...
        mlflow_logger.metrics_summary(metrics, 1)
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_iterator):
                ...
                output_dic = model(input_ids=input_ids,
                                   ...
                                   time_masked_lm_labels=time_masked_lm_labels,
                                   tkg_tuple=tkg_tuple,
        ...
        mlflow_logger.metrics_summary(test_metrics, 1)
        mlflow_logger.artifact_summary(os.path.join(args.log_file_dir, log_file))
